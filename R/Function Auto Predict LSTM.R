#' Forecast Multiple Time Series using Customizable LSTM Models
#'
#' This function builds an independent LSTM model for each input time series
...
#' @param lstm_layer_units A numeric vector specifying the number of neurons for each LSTM layer. 
...
autopredictlstm <- function(WT, SM, Rf, Temp, h, 
                           look_back = 12, 
                           lstm_layer_units = c(50), 
                           epochs = 100) {
  ...
  for (col_name in columns) {
    ...
    # Build the Customizable LSTM Model
    inputs <- layer_input(shape = c(look_back, 1))
    
    # Dynamically build the LSTM layers based on user input
    x <- inputs
    num_layers <- length(lstm_layer_units)
    for (i in 1:num_layers) {
      units <- lstm_layer_units[i]
      # All layers except the last one must return sequences
      return_sequences_flag <- (i < num_layers)
      
      x <- x %>% layer_lstm(units = units, return_sequences = return_sequences_flag)
    }
    ...
    cat("Training LSTM model with architecture:", lstm_layer_units, "\n")
    ...
    plot(series_data, type = "l", col = "blue", lwd = 2,
         main = paste("LSTM Forecast -", name_map[col_name]),
